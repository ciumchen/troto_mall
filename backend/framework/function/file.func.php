...
function file_upload($file, $type = 'image', $name = '') {
	...
	if(!in_array($type, array('image','audio','file'))) {
	...
	if (empty($_W['uploadsetting'][$type])) {
		...
		$_W['uploadsetting'][$type]['folder'] = "{$type}s";
	...
	if(empty($name) || $name == 'auto') {
		$result['path'] = "{$settings[$type]['folder']}/" . date('Ym/');
	...
	if (filesize($file['tmp_name'])>850*1024) {
		load()->class('imgcompress');
		(new ImgCompress(ATTACHMENT_ROOT.'/'.$result['path'],1))->compressImg(ATTACHMENT_ROOT.'/'.$result['path']);
	}

	$result['path'] = $_W['config']['upload']['cdnuri'].$result['path'];
	$result['success'] = true;
	return $result; 
}
 ...
 * @param format
 */
function file_getFiledate($file, $format) {
    if(is_file($file)) {
    	$filePath = $file;
