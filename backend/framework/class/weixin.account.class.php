...
class WeiXinAccount extends WeAccount {
	...
	public function fetch_token() {
		global $_W;
		$token = ihttp_get($_W['config']['access_token_url']);
		return  ($token['code']=='200') ? $token['content'] : FALSE;
	...
	function sendCustomNotice($data,$type=true) {
		load()->func('communication');
		if(empty($data)) {
			return error(-1, '参数错误');
		}
	...
	/**
	 * 发送模板消息
	 * array('touser'=>'','template_id'=>'','url'=>'','topcolor'=>'','data'=>array('first'=>array('value'=>'','color'=>''),'keynote1'=>array('value'=>'','color'=>''),'remark'=>array('value'=>'','color'=>'')));
	 */
	...
	public function getTemplateId($data){
		...
		$post_url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={$token}";
		$response = ihttp_post($post_url, json_encode($data));
		if(is_error($response)) {
			return error(-1, "访问公众平台接口失败, 错误: {$response['message']}");
		}
		return $response;
