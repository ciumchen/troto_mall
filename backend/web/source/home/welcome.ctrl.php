...
$dos = array('platform', 'site', 'mc', 'setting', 'ext', 'solution','test');
...
if($do == 'mc') {
	...
	$num = pdo_fetchcolumn('SELECT COUNT(uid) FROM '.tablename('members'));
...
if($do == 'test'){
	$sql = 'select uid from ims_members ';
	$user = pdo_fetchAll($sql);
	foreach($user as $key => $value){
		$relation = getPid($value['uid'],3);
		$insert = array('uid'=>$value['uid']);
		$insert['uid1'] = $relation[1]?$relation[1]:0;
		$insert['uid2'] = $relation[2]?$relation[2]:0;
		$insert['uid3'] = $relation[3]?$relation[3]:0;
