<?php
define('IN_SYS', true);
define('BACKEND_URL', 'http://adm.troto.com.cn/');
define('DEFAULT_UNIAACID', 16);
...
if(!defined('IN_GW')) {
	// checkaccount();
	if(empty(uni_permission($_W['uid'], DEFAULT_UNIAACID))) {
		message('操作失败, 非法访问.');
	}
	uni_group_check();
	if(!in_array($_W['role'], array('manager', 'operator', 'founder'))) {
		message('您的账号没有访问此公众号的权限1.');
	}
}
require _forward($controller, $action);
...
function _forward($c, $a) {
	...
	if($_W['user']['sid']){
		if($c == 'mc' || $c == 'home'){
			header('location:./index.php?c=site&a=entry&eid=2');
		}
	}
	return $file;
