 ...
 *
 */
$_W['adminpower'] = array(PRODUCT=>'产品权限',ORDER=>'订单权限',ACCOUNT=>'粉丝权限',FINANCE=>'财务权限',UPD=>'修改权限',ADMINISTRATOR=>'ADMIN');
/**
 * 设置合用性
 */
// $_W['usability'] = array('hidden'=>1,'developer'=>1);
$_W['usability'] = array('hidden'=>0,'developer'=>0);
...
$_W['siteroot'] = $_W['sitecheck'] = htmlspecialchars('http://' . $_SERVER['HTTP_HOST'] . $sitepath);
if(substr($_W['siteroot'], -1) != '/') {
	$_W['siteroot'] .= '/';
}

# 管理后台域名登录设置前台域名
if($_W['script_name'] === '/web/index.php' && $_W['siteroot'] === 'http://adml.yunji007.com/web/'){
// if($_W['script_name'] === '/web/index.php' && $_W['siteroot'] === 'http://w.ichibanv2.com/web/'){
	$_W['siteroot'] = 'http://ichiban.yunji007.com/web/';
}
