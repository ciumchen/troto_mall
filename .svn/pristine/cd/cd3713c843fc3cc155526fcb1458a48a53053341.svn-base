...
if(!empty($log) && $log['status'] != '0') {
	...
	message("这个订单已经支付成功, 不需要重复支付!", $_W['siteroot'].'app/'.substr(url('entry/index/myorder', array('status'=>5,'m'=>'ewei_shopping')),2));
}
...
document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
	WeixinJSBridge.invoke('getBrandWCPayRequest', {
		...
		if(res.err_msg == 'get_brand_wcpay_request:ok') {
			alert('支付成功');
			window.location.href = "<?php echo $_W['siteroot'].'app/'.substr(url('entry/index/single', array('m'=>'ewei_shopping')),2); ?>";
			// window.location.href = "<?php echo $_W['siteroot'].'app/'.substr(url('entry/index/myorder', array('status'=>2,'m'=>'ewei_shopping')),2); ?>";
			// location.search += '&done=1';
		} else {
			alert('支付失败');
			// alert('启动微信支付失败, 请检查你的支付参数. 详细错误为: ' + res.err_msg);
			// window.location.href = "<?php echo $_W['siteroot'].'app/'.substr(url('entry/index/single', array('m'=>'ewei_shopping')),2); ?>";
			window.location.href = "<?php echo $_W['siteroot'].'app/'.substr(url('entry/index/myorder', array('m'=>'ewei_shopping')),2); ?>";
