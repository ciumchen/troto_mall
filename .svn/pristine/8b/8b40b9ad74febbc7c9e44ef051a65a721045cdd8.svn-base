...
function mc_check($data) {
	...
	if(!empty($data['email'])) {
		...
		$isexist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('members') . ' WHERE uniacid = :uniacid AND email = :email AND uid != :uid', array(':uniacid' => $_W['uniacid'], ':email' => $email, ':uid' => $_W['member']['uid']));
		if($isexist >= 1) {
	...
	if(!empty($data['mobile'])) {
		...
		$isexist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('members') . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid != :uid', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile, ':uid' => $_W['member']['uid']));
		if($isexist >= 1) {
...
function mc_update($uid, $fields) {
	...
	$isexists = pdo_fetchcolumn("SELECT uid FROM ".tablename('members')." WHERE uid = :uid", array(':uid' => $uid));
	$condition = '';
	if(!empty($isexists)) {
		$condition = ' AND uid != ' . $uid;
	}
		if(!empty($fields['email'])) {
		$emailexists = pdo_fetchcolumn("SELECT email FROM ".tablename('members')." WHERE uniacid = :uniacid AND email = :email " . $condition, array(':uniacid' => $_W['uniacid'], ':email' => trim($fields['email'])));
		if($emailexists) {
			unset($fields['email']);
		}
	} 
	if(!empty($fields['mobile'])) {
		$mobilexists = pdo_fetchcolumn("SELECT mobile FROM ".tablename('members')." WHERE uniacid = :uniacid AND mobile = :mobile " . $condition, array(':uniacid' => $_W['uniacid'], ':mobile' => trim($fields['mobile'])));
		if($mobilexists) {
			unset($fields['mobile']);
		}
	}
	...
	if (empty($isexists)) {
		...
		pdo_insert('members', $fields);
		return pdo_insertid();
	} else {
		$result = pdo_update('members', $fields, array('uid' => $uid));
		return $result > 0;
	}
}
...
function mc_fetch($uid, $fields = array()) {
	...
	if(is_array($uid)){
		$result = pdo_fetchall("SELECT $select FROM ".tablename('members')." WHERE uid IN ('".implode("','", is_array($uid) ? $uid : array($uid))."')", array(), 'uid');
		...
		$result = pdo_fetch("SELECT $select FROM " . tablename('members') . " WHERE `uid` = :uid", array(':uid' => $uid));
 ...
 * @param openid
 */
function mc_fansNickname($openid, $acid = 0){
	global $_W;
	...
	$sql = 'SELECT nickname, tag FROM ' . tablename('mc_mapping_fans') . $condition .' LIMIT 1';
	...
	if(!empty($fan)){
		if (!empty($fan['tag']) && is_string($fan['tag'])) {
			...
			if (is_serialized($fan['tag'])) {
				$fan['tag'] = @iunserializer($fan['tag']);
				if(!empty($fan['tag']['nickname']) && empty($fan['nickname'])){
	...
	if(!empty($message)) {
		...
		if (checksubmit('submit')) {
			...
			if(in_array('email', $fields)) {
				$emailexists = pdo_fetchcolumn("SELECT email FROM ".tablename('members')." WHERE uniacid = :uniacid AND email = :email " . $condition, array(':uniacid' => $_W['uniacid'], ':email' => trim($record['email'])));
				if(!empty($emailexists)) {
					message('抱歉，您填写的手机号已经被使用，请更新。', 'refresh', 'error');
				}
			}
			if(in_array('mobile', $fields)) {
				$mobilexists = pdo_fetchcolumn("SELECT mobile FROM ".tablename('members')." WHERE uniacid = :uniacid AND mobile = :mobile " . $condition, array(':uniacid' => $_W['uniacid'], ':mobile' => trim($record['mobile'])));
 ...
 *
 */
function mc_credit_update($uid, $credittype, $creditval = 0, $log = array(), $bonus = 0) {
	global $_W;
	...
	// $value = pdo_fetchcolumn("SELECT $credittype FROM ".tablename('members')." WHERE `uid` = :uid", array(':uid' => $uid));
	$value = pdo_fetch("SELECT $credittype, credit1, credit7 FROM ".tablename('members')." WHERE `uid` = :uid", array(':uid' => $uid));

	if($creditval > 0 || ($value[$credittype] + $creditval >= 0)) {
		# credit2 余额，credit1 红包，credit7 总余额
		$mcData =array($credittype => $value[$credittype] + $creditval);

		# 不等于 0 可操作：增减红包字段的value
		if($bonus != 0 && $credittype != 'credit1'){
			$mcData['credit1'] = $value['credit1'] + $bonus;
		}
		# 不少于 0 时操作：credit7只能增不能减
		if($creditval > 0){
			$mcData['credit7'] = $value['credit7'] + $creditval;
		}
		pdo_update('members', $mcData, array('uid' => $uid));	
...
function mc_credit_fetch($uid, $types = array()) {
	if(empty($types) || $types == '*') {
		$select = 'credit1, credit2, credit3, credit4, credit5, credit7';
	} else {
	...
	return pdo_fetch("SELECT {$select} FROM ".tablename('members').' WHERE uid = :uid LIMIT 1',array(':uid' => $uid));
}
...
/**
 * 会员信息获取，并登录
 * @param arr $user = array('uid' => UID);
 */
function _mc_login($user) {
	global $_W;
	if(!empty($user) && !empty($user['uid'])) {
		$condition = array('uid', 'credit1', 'credit2', 'credit3', 'credit3results','credit5', 'mobile', 'email', 'nickname', 'realname', 'createtime', 'pid', 'avatar');
		$user = mc_fetch($user['uid'], $condition);		
		if(!empty($user) && (!empty($user['mobile']) || !empty($user['email']))) {
			...
			if(empty($_W['openid'])) {
				...
				$fan = mc_fansinfo($_W['openid'], $_W['acid']);
				if(empty($fan['uid']) && !empty($fan['acid'])) {
					...
					if(!empty($fan['tag']['nickname'])){
						$_W['member']['nickname'] = $fan['tag']['nickname'];
					}
				}
			}
 ...
 *
 */
function mc_getUserInfoToExchange(){
	global $_W;
	return pdo_fetch("SELECT 
						u.credit3, u.credit5, b.bankid, b.fbankname, b.fbanklocation, b.fbankaccount, b.fusername, b.fmobile
					 FROM 
					 	ims_members u LEFT JOIN ims_mc_bank b on u.uid = b.uid 
					 WHERE 
					 	b.ftype = 1 AND u.uid = :uid
					 ORDER BY b.fdefault DESC LIMIT 1", array(':uid' => $_W['member']['uid']));
}
 ...
 *
 */
function mc_getUserSign(){
	global $_W;
	return pdo_fetch('SELECT u.avatar, u.nickname, u.realname, u.createtime, s.counttimes FROM '.
					  tablename('members').' u left join '.tablename('mc_member_sign').' s on u.uid = s.uid where u.uid = :uid and u.uniacid = :uniacid', 
 ...
 * @param int $id
 */
function mc_getNickname($id){
	global $_W;
	if(is_numeric($id)){
		$res = pdo_fetch('select nickname from '.tablename('members').' where uid = :uid', array(':uid' => $id));
		if(empty($res['nickname'])){
			$res = mc_fansNickname($id);
		}
	}else{
		$res = mc_fansNickname($id);;
	}
	return empty($res['nickname']) ? '':$res['nickname'];
}
