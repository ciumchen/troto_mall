...
if ($do == 'upload') {
	if($type == 'image'){
		...
		if (!empty($_FILES['file']['name'])) {
			...
			if(empty($_GPC['mediatype'])){
				$_W['uploadsetting'] = array();
				$_W['uploadsetting']['audio']['folder'] = 'audios';
...
if ($do == 'browser') {
	...
	if($type == 'image') {
		...
		if(!empty($option['global'])){
			...
			$browser = 'images';
			$parentbrowser = 'images';
			
			if(empty($path)){
				$rootpath = IA_ROOT .'/'.$_W['config']['upload']['attachdir'].'/'.'images';
				...
				$rootpath = ATTACHMENT_ROOT. '/' .'images';
				$currentpath = $rootpath;
				if(!empty($yearmonth)){
					$currentpath .= '/'.$yearmonth;
					$browser .= '/'.$yearmonth;
				}
			}
		}
			...
			if ($handle = opendir($currentpath)) {
				while (false !== ($filename = readdir($handle))) {
					...
					if (is_dir($file)) {
						...
						$entry = array();
						$entry['url'] = $_W['attachurl'].'images/'.$year.$month.'/'.$filename;
						$entry['filename'] = 'images/'.$year.$month.'/' . $filename;
						if($option['global']){
	...
	if($type == 'audio') {
		...
		$rootpath = IA_ROOT .'/'.$_W['config']['upload']['attachdir'].'/'.'audios';
		$exts = array('mp3');
