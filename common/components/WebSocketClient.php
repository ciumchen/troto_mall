...
class WebSocketClient {
     ...
     * @param string $path
     */
    function __construct($host='127.0.0.1', $port=8080, $deviceId='', $secret='', $path = '/', $origin = null) {
        $this->host   = $host;
        $this->port   = $port;
        $this->path   = $path;
        $this->origin = $origin;
        $this->device = $deviceId;
        $this->key    = base64_encode(substr(md5($secret.date('YmdHi')), 8 ,16));
    ...
    public function send($data, $type = 'text', $masked = false) {
        switch($type) {
            case 'text': $_type = WEBSOCKET_OPCODE_TEXT; break;
            case 'binary':
            case 'bin': $_type = WEBSOCKET_OPCODE_BINARY; break;
    ...
    private function createHeader() {
        ...
        "Sec-WebSocket-Device: {$this->device}" . "\r\n" .
        "Sec-WebSocket-Secret: {$this->key}" . "\r\n" .
